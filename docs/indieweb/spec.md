...
| Webmention sending      | When a published post links to a page that advertises a Webmention endpoint, the sender discovers that endpoint and submits the source and target. A changed post can be sent again.                                                                                                                                                                                                                                                                                                                                                                                            | Unit checks cover discovery, tracking, and request data. Use Webmention Rocks sender cases from a published test post and inspect the receiver's recorded result.                                                                                                                                                                   |
...
| IndieAuth and Micropub  | The site advertises its own IndieAuth metadata, authorization endpoint, and token endpoint. Micropub creates a note, photo, article, reply, like, repost, bookmark, or RSVP only with an unexpired `create` token for this profile. A successful create returns `Location`, writes content through the configured storage path, and yields a published permalink after deployment. Missing and invalid tokens create nothing. A duplicate token in the header and body receives HTTP 400. The `media-endpoint` appears in configuration only when upload storage is configured. | Unit checks cover authorization, token lifecycle, request parsing, storage, duplicate authentication, and conditional media discovery. The HTTP suite checks discovery and rejection. An isolated write test must obtain a token, create a post, and verify the commit, permalink, and feed entry. Run Micropub Rocks server cases. |
| WebSub publishing       | Each RSS and Atom feed advertises its hub and self URL in XML. Each JSON Feed advertises them in HTTP `Link` headers. A publish notification names each changed feed.                                                                                                                                                                                                                                                                                                                                                                                                           | Unit checks cover feed output and the ping request. The HTTP suite fetches the feeds and checks JSON headers. Run WebSub Rocks publisher discovery against the test deployment.                                                                                                                                                     |
